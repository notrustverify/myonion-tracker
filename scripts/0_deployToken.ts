...
import { MintSupply, TokenTest } from '../artifacts/ts';
...
const deployScript: DeployFunction<Settings> = async (
  ...
  const result = await deployer.deployContract(TokenTest, {
    initialFields: {
        name: stringToHex('Test'),
        symbol: stringToHex('T'),
        supply: 1000_000000_000000_000000n
    }
  })
